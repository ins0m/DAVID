SELECT d.subject_id AS "subject",
       p.block,
       p.task,
       stddev(d."Driver’s lateral lane position with respect to the roadway di") AS "sdlp"
FROM "2013-sim-c".simdata d INNER JOIN "2013-sim-c".protocol_nback p
ON (
...
group by d.subject_id, p.block, p.task
